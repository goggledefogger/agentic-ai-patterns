...
**The runner also imports context you never wrote, and that alone can exceed the window.** Measured 2026-09-22 in a dashboard's local-fallback chat (a 9B model at 49K context behind an OpenAI-compatible server): run inside a project folder, the runner silently loaded the user's global instruction file, the project's instruction file, and a listing of every installed skill on the machine (about 200). The first request was 205 KB, 170 KB of it system prompt. The server answered HTTP 400 in under 2 s; the runner auto-compacted and retried, and kept doing that for 15 minutes (13,000+ streamed deltas of compaction summaries, no answer). A fast 400 followed by an endless compaction loop is the signature. Two fixes, both needed. Switch off every auto-import the runner offers, in the env of that one process only (for opencode: `OPENCODE_DISABLE_CLAUDE_CODE=1`, `OPENCODE_DISABLE_EXTERNAL_SKILLS=1`, `OPENCODE_DISABLE_AUTOCOMPACT=1`; the request fell to 57 KB and the first correct reply took 98 s where the day before had taken about 4 min). And measure the actual request instead of reasoning about it: a 20-line stand-in HTTP server on a spare port that saves each request body and returns 400, named as the provider's `baseURL`, shows the exact prompt without loading a model.
...
- **[`always-loaded-context-budget.md`](always-loaded-context-budget.md)**: the same auto-injected instruction files as a budget on a frontier harness. On a local model the runner's import of them is the first thing to switch off (trap 1).
